//
//  NetworkManger.swift
//  BusTimes-MVVM
//
//  Created by Ade Adegoke on 05/02/2023.
//

import Foundation
import Combine

class NetworkManager {
    private let session: URLSession
    private let decoder: JSONDecoder
    
    init(session: URLSession = .shared, decoder: JSONDecoder = .init()) {
        self.session = session
        self.decoder = decoder
    }
}

extension NetworkManager: Networkable {
    func fetchData<T>(endpoint: Endpoints) -> AnyPublisher<T, APIError> where T : Decodable {
        
        guard let url = endpoint.url else { preconditionFailure("Can't create url for query: \(endpoint)") }
        
        return session.dataTaskPublisher(for: url)
            .receive(on: DispatchQueue.main)
            .mapError { APIError.network($0) }
            .map { $0.data }
            .decode(type: T.self, decoder: decoder)
            .mapError { _ in APIError.noData }
            .map { $0 }
            .eraseToAnyPublisher()
        
    }
    
    func fetchData<T>(using string: String) -> AnyPublisher<T, APIError> where T : Decodable {
        let url = "https://api.tfl.gov.uk/StopPoint/\(string)//arrivals"
        guard let busIdURL = URL(string: url) else { preconditionFailure("Can't create url for query: \(string)") }
        return session.dataTaskPublisher(for: busIdURL)
            .receive(on: DispatchQueue.main)
            .mapError { APIError.network($0) }
            .map { $0.data }
            .decode(type: T.self, decoder: decoder)
            .mapError { error in APIError.network(error) }
            .map { $0 }
            .eraseToAnyPublisher()
    }
    
    
}
